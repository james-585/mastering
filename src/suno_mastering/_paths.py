...
does `Path(__file__).resolve().parents[2]` to find the repo root -- config.py
...
their bare-module code could be imported. That stitching is gone (removed in
the 2026-09-12 restructure, then the package itself moved to src/suno_mastering/
on 2026-09-12): those story modules now live as proper submodules and
subpackages under suno_mastering/ (stem_stages/, quality_review/, tools/)
and are imported normally. bundle_root()/frozen_root() are still needed for
the remaining, unrelated data-file-location uses above.

In a normal source checkout, "repo root" is that parents[2] directory
(src/suno_mastering/_paths.py -> src/suno_mastering -> src -> repo root) as
